...
Optionally, remove the “rhgb quiet” kernel parameters for every node, so you can easily see which cluster daemon fails to start on VM bootup.
  ...
  3. Use a web browser to access the web-based HA configuration tool luci -- first login with root and grant the user ricci the permission to administrator and create a cluster, then logout and login with ricci.

 
  4. Create a 5-node cluster “my-cluster”. We can confirm the cluster is created properly by checking the status of the service daemons and checking the cluster status (clustat): 
```code
