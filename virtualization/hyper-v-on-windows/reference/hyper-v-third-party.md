...
|[Unrecoverable Exception](hyper-v-third-party-funcs/EnrecoverableException.md)|An exit for an unrecoverable error is caused by the virtual processor generating an exception that cannot be delivered (triple fault). |
...
|[Execution Cancelled](hyper-v-third-party-funcs/ExecutionCancelled.md)|Information about an exit caused by host system is provided in the `WHV_RUN_VP_CANCELLED_CONTEXT` structure.  |
|   |   |
