...
```none
Invoke-WebRequest "https://test.docker.com/builds/Windows/x86_64/docker-1.13.0-rc5.zip" -OutFile "$env:TEMP\docker-1.13.0-rc5.zip" -UseBasicParsing
```

Expand the zip archive into Program Files, the archive contents is already in docker directory.

```none
Expand-Archive -Path "$env:TEMP\docker-1.13.0-rc5.zip" -DestinationPath $env:ProgramFiles
```
