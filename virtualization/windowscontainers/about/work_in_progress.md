...
Many anti-malware solutions, such as Windows Defender, may be unnecessarily scanning files with-in container images including all of the OS binaries and files in the container OS image.  This occurs whenever a new container is created and from the anti-malware’s perspective all of the “container’s files” look like new files that have not previously been scanned.  So when processes inside the container attempt to read these files the anti-malware components first scan them before allowing access to the files.  In reality these files were already scanned when the container image was imported or pulled to the server. From Technical Preview 5, the infrastructure is available in place such that anti-malware solutions, including Windows Defender, will be aware of the situations and can act accordingly to avoid multiple scans. Anti-malware solutions can update their solution using the guidance described here -  https://msdn.microsoft.com/en-us/windows/hardware/drivers/ifs/anti-virus-optimization-for-windows-containers and avoid multiple scans. 
