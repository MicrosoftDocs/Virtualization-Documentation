...
If you are creating a network, you can specify the name that it is given by the HNS service using the option, *-o com.docker.network.windowsshim.networkname=<network name>* option to the `docker network create` command. For instance, you might use the following command to create a transparent network with a name that is specified to the HNS service:

```none
C:\> docker network create -d transparent -o ccom.docker.network.windowsshim.networkname=ExampleTransparentNet ExampleTransparentNet
```
