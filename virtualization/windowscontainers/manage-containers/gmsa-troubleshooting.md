...
### Using a gMSA with more than one container simultaneously leads to intermittent failures on Windows Server 2019 versions and above if the --hostname parameter is specified

Because all containers using the gMSA account when multiple containers are assigned the same identity and in the `--hostname` parameter the gmsa account name is specified, a race condition may occur when two containers talk to the same domain controller simultaneously. When another container talks to the same domain controller, it will cancel communication with any prior containers using the same identity.

This can lead to intermittent authentication failures and can sometimes be observed as a trust failure when you run nltest /sc_verify:contoso.com inside the container. To avoid this issue in the docker run command if the --hostname parameter is specified then it must be always unique when running the container simultaneously with the gmsa account. For example, if the gmsa account is webapp01.contoso.com and if two containers are running simultaneously then both containers can have `--hostname` parameter "webapp01" , "webapp02" respectively.
